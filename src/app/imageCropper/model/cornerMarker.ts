...
export class CornerMarker extends Handle implements ICornerMarker {
    ...
    public drawCornerBorder(ctx: CanvasRenderingContext2D): void {
        ...
        if (this.cropperSettings.rounded) {
            const width: number = this.position.x - this.horizontalNeighbour.position.x;
            const height: number = this.position.y - this.verticalNeighbour.position.y;

            const offX: number = Math.round(Math.sin(Math.PI / 2) * Math.abs(width / 2)) / 4;
            const offY: number = Math.round(Math.sin(Math.PI / 2) * Math.abs(height / 2)) / 4;
