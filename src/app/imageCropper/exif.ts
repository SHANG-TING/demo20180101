...
export class Exif {
    ...
    public static getImageData(img: IImageExtended | Blob | File, callback: Function) {
        function handleBinaryFile(binFile: ArrayBuffer) {
            const data = Exif.findEXIFinJPEG(binFile);
            const iptcdata = Exif.findIPTCinJPEG(binFile);
        ...
        if ('src' in img && (img as IImageExtended).src) {
            if (/^data:/i.test((img as IImageExtended).src)) { // Data URI
                const arrayBuffer = Exif.base64ToArrayBuffer((img as IImageExtended).src);
    ...
    public static findIPTCinJPEG(file: ArrayBuffer) {
        ...
        const isFieldSegmentStart = function (_dataView: DataView, _offset: number) {
            return (
                ...
                if ({}.hasOwnProperty.call(exifData, tag)) {
                    switch (tag) {
                        ...
                        case 'ComponentsConfiguration' :
                            ...
                            exifData[tag] = Exif.StringValues[compopents][exifData[tag][0]]
    ...
    public static getAllTags(img: any) {
        ...
        const data: any = img.exifdata, tags: any = {};
        let a: string;
        for (a in data) {
