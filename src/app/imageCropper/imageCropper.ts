...
export class ImageCropper extends ImageCropperModel {
    ...
    public prepare(canvas: HTMLCanvasElement) {
        ...
        const responsiveWidth: number = canvas.parentElement ? canvas.parentElement.clientWidth : 0;
    ...
    public getSide(a: Point, b: Point, c: Point): number {
        const n: number = ImageCropper.sign((b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x));
    ...
    public handleMove(newCropTouch: CropTouch) {
        ...
        for (let k = 0; k < this.currentDragTouches.length; k++) {
            if (newCropTouch.id === this.currentDragTouches[k].id && this.currentDragTouches[k].dragHandle != null) {
                ...
                const clampedPositions = this.clampPosition(newCropTouch.x - dragTouch.dragHandle.offset.x,
        ...
        if (!matched) {
            for (let i = 0; i < this.markers.length; i++) {
                const marker: ICornerMarker = this.markers[i];
    ...
    public setImage(img: any) {
        ...
        if (!img) {
            ...
            const bufferContext = <CanvasRenderingContext2D> this.buffer.getContext('2d');
    ...
    private getCropPositionFromBounds(cropPosition: Bounds): Point[] {
        ...
        const ratioW: number = (this.canvasWidth - marginLeft * 2) / this.srcImage.width;
        const ratioH: number = (this.canvasHeight - marginTop * 2) / this.srcImage.height;
        ...
        const tlPos: Point = PointPool.instance.borrow(actualX, actualY + actualH);
        const trPos: Point = PointPool.instance.borrow(actualX + actualW, actualY + actualH);
        const blPos: Point = PointPool.instance.borrow(actualX, actualY);
        const brPos: Point = PointPool.instance.borrow(actualX + actualW, actualY);
        const center: Point = PointPool.instance.borrow(actualX + actualW / 2, actualY + actualH / 2);
    ...
    public getCroppedImage(preserveSize?: boolean, fillWidth?: number, fillHeight?: number): HTMLImageElement {
        ...
        if (!this.srcImage) {
            ...
            const ctx = <CanvasRenderingContext2D> this.cropCanvas.getContext('2d');
        ...
        if (this.crop.isImageSet()) {
            ...
            if (event.touches.length === 1) {
                for (let i = 0; i < event.touches.length; i++) {
                    ...
                    const touchPosition = ImageCropper.getTouchPos(this.canvas, touch);
                ...
                if (event.touches.length === 2) {
                    ...
                    if (this.previousDistance && this.previousDistance !== distance) {
                        ...
                        if (distance > this.previousDistance) {
                            if (bounds.top !== this.minYClamp && bounds.bottom !== this.maxYClamp &&
                                bounds.left !== this.minXClamp && bounds.right !== this.maxXClamp) {
                                // none
                                bounds.top -= 1;
                                bounds.left -= 1;
                                bounds.right += 1;
                                bounds.bottom += 1;
                            } else if (bounds.top !== this.minYClamp && bounds.bottom !== this.maxYClamp &&
                                       bounds.left === this.minXClamp && bounds.right !== this.maxXClamp) {
                                // left
                                bounds.top -= 1;
                                bounds.right += 2;
    ...
    public onMouseMove(e: MouseEvent) {
        ...
        if (this.crop.isImageSet() && this.isMouseDown) {
            const mousePosition = ImageCropper.getMousePos(this.canvas, e);
